...
class Elfin:
    ...
    def ReadAcsActualPos(self):
        ...
        :return:
            ...
            if Error Return False
        """
        message = "ReadAcsActualPos," + self.robot_id + self.end_msg
        coord = self.send(message)
        if coord:
            return [float(s) for s in coord]
    ...
    def MoveJ(self, target):
        ...
        target = [str(s) for s in target]
        target = (",".join(target))
        message = "MoveJ," + self.robot_id + ',' + target + self.end_msg
        return self.send(message)

    def MoveZ(self, target):
        """
        function: Arc motion
        :param: coordStart[6]+coordEnd[6]+coordVia[6]+dWidth+nMoveZBaseCoord+dDensity+nUseZDensityint,;
        :return:
        ...
        target = [str(s) for s in target]
        target = (",".join(target))
        message = "MoveZ," + self.robot_id + ',' + target + self.end_msg
        return self.send(message)

    def StopingRobot(self, joint):
        limit = [330,120,120,330,160,330]
        degreeJ = self.ReadAcsActualPos()[joint]
        if degreeJ >= limit[joint] or degreeJ <= -limit[joint]:
            self.GrpStop()
            return True
        return False
    ...
    def MoveRelL(self, coordinate, direction, distance):
        ...
        target = [str(s) for s in target]
        target = (",".join(target))
        message = "MoveRelL," + self.robot_id + ',' + target + self.end_msg
        return self.send(message)
    ...
    def MoveRelJ(self, coordinate, direction, distance):
        ...
        target = [str(s) for s in target]
        target = (",".join(target))
        message = "MoveRelJ," + self.robot_id + ',' + target + self.end_msg
        return self.send(message)
    ...
    def CalibrationJ(self, coordinate, direction, distance):
        self.MoveRelJ(coordinate, direction, distance)
        status = self.ReadMoveState()
        while status == 1009:
            sleep(0.5)
            status = self.ReadMoveState()
